...
func worker(id int, jobs <-chan int, results chan<- int) { // 这是我们将要在多个并发实例中支持的任务了。
...
func main() {
	jobs := make(chan int, 100) // 为了使用 worker 工作池并且收集他们的结果，我们需要2个通道
	results := make(chan int, 100)

	for w := 1; w <= 3; w++ { // 这里启动了 3 个 worker，初始是阻塞的，因为还没有传递任务。
	...
	for a := 1; a <= 9; a++ { // 最后，我们收集所有这些任务的返回值。
		<-results
	}
